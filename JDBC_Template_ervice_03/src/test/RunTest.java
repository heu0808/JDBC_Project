...
public class RunTest {
	...
	* -ResultSet : SELECT문 실행 후 조회된 결과물들이 담겨있는 객체
	...
	4) sql문 전달하면서 실행 : Statement 객체를 이용해서 sql문 실행 
 	...
 	5) 결과받기 > SELECT문 실행 -> ResultSet객체 (조회된 데이터들이 담겨있음)
		 * 
		 * 6-1) > DML문 실행 -> int(처리된 행 수)
		 * 
		 * 6-1) ResultSet에 담겨있는 데이터들을 하나씩 하나씩 뽑아서 vo객체에 차근차근 옮겨담기[+ ArrayList에 담아주기]
		 * 6-2) 트랜젝션 처리(성공했다면 commit, 실패했다면 rollback 실행)
		 * 
		 ...
		 * 수(int) => 트랜잭션
		 * 
		 * // 필요한 변수 먼저 세팅 int result = 0; 
		 * //결과(처리된 행수)를 받아줄 변수 
		 * Connection conn = null;
		 * //DB에 연결정보를 보관할 객체
		 *  Statement stmt = null; 
		 * //sql문 전달해서 실행 후 결과를 받는 객체
...
//		4, 5) sql문 전달해서 실행 후 결과 받기(ResultSet객체)
		...
		//rest.next() => rest의 다음행이 들어있는지 없는지 확인 후 결과 반환 + 한행을 내려준다
		
		while(rset.next()) {
//			현재 참조하는 rset으로부터 어떤 컬럼에 해당하는 값을 어떤타입으로 뽑을건지 제시해줘야한다.
//			DB의 컬럼명 제시(대소문자 가리지 않는다.)
